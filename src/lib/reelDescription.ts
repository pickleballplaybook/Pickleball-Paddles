...
export interface ClipForDescription {
  title?: string;
  // Social post body — what viewers read under the Reel. Generated by Claude
  // alongside the title (see shorts-backend/server.js prompt). Preferred over
  // `reason` when present; older clips without a description fall back to
  // reason so we don't break legacy meta.json files.
  description?: string;
  // Internal rationale for the admin UI ("why we picked this clip"). Kept
  // around for backward compat — older clip jobs only have this field, so
  // buildReelDescription uses it as a fallback when description is missing.
  reason?: string;
}
...
export function buildReelDescription(
  ...
  // Prefer the dedicated `description` (social-post body). Fall back to the
  // older `reason` (internal rationale) so legacy clips without a
  // description still produce a non-empty post.
  const body = (clip.description && clip.description.trim()) || (clip.reason && clip.reason.trim());
