...
type Clip = {
  ...
  path: string;
  title?: string;
  // Social post body — what gets posted as the Reel / Short caption.
  // Generated by Claude alongside the title, takes precedence over `reason`
  // in buildReelDescription.
  description?: string;
  // Internal "why we picked this clip" rationale. Older clips only have this
  // field; buildReelDescription falls back to it when description is empty.
  reason?: string;
  duration?: number;
  createdAt?: string;
  sourceUrl?: string;
...
type BrandKey = "playbook" | "drills" | "picklebrain" | "paddles" | "other";

const BRAND_LABEL: Record<BrandKey, string> = {
  playbook:    "Pickleball Playbook",
  drills:      "Pickleball Drills",
  picklebrain: "Picklebrain",
  paddles:     "Pickleball Playbook Paddles",
  other:       "Other",
};

const BRAND_ORDER: BrandKey[] = ["playbook", "drills", "picklebrain", "paddles", "other"];

// Match brand by keywords in the account name / username. Checked in order
// of specificity — "paddles" (and its legacy alias "review") is tested
// BEFORE "playbook" because every Paddles account also contains "playbook"
// in its name (e.g. "Pickleball Playbook Paddles"), so a plain "playbook"
// check would swallow it into the Playbook column and hide the split.
function brandFor(label: string): BrandKey {
  const s = label.toLowerCase();
  if (s.includes("paddle") || s.includes("review"))     return "paddles";
...
export default function PublishPage() {
  ...
  // Dirty flags — flipped true only when the USER types in the field. The
  // string-equality guard pattern that used to live here was unreliable
  // across publish cycles (small whitespace drift between renders broke
  // the comparison), so we track edits explicitly. Reset to false on
  // successful publish or when the user picks a clip.
  ...
  useEffect(() => {
    ...
    if (next === null) return;
    // Always overwrite unless the user has typed something themselves.
    if (!descriptionEdited) setDescription(next);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sourceMode, selectedFile, selectedClipPath, clips]);

  // ── Auto-fill the TITLE when a clip is selected ─────────────────────────
  // Same dirty-flag pattern as the description. Upload mode keeps the title
  // blank — uploads have no per-video title metadata.
  useEffect(() => {
    if (sourceMode !== "clip" || !selectedClipPath || !clips) return;
    const clip = clips.find((c) => c.path === selectedClipPath);
    if (!clip || !clip.title) return;
    if (!titleEdited) setTitle(clip.title);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sourceMode, selectedClipPath, clips]);

  // Picking a different clip clears any prior edits — the user is starting
  // fresh, so the next clip's auto-fill should win. Without this, a user who
  // edited the description for clip A and then switched to clip B would see
  // clip A's edited text (since descriptionEdited stays true).
  useEffect(() => {
  ...
  async function handlePublish() {
    ...
    try {
      ...
      // Clip mode: reset the form so the next clip the user picks gets a
      // fresh auto-fill instead of inheriting the just-published clip's
      // (possibly manually-tweaked) title + description. Clearing
      // selectedClipPath also fires the dirty-flag reset effect, but we
      // null both flags here explicitly for paranoia in case state batching
      // re-orders things across React versions.
      if (sourceMode === "clip") {
        setSelectedClipPath("");
        setTitle("");
        setDescription("");
          ...
          <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
            ...
            <div className="flex gap-1.5 items-center flex-wrap">
              <FilmerCreditPicker
                ...
                onChange={(v) => { setDescription(v); setDescriptionEdited(true); }}
              />
              {DESCRIPTION_PRESETS.map((p) => (
                <button
                  key={p.label}
                  type="button"
                  onClick={() => { setDescription(p.text); setDescriptionEdited(true); }}
          ...
          {connections === null ? (
            ...
            (() => {
              ...
              const grouped: Record<BrandKey, Row[]> = {
                playbook: [], drills: [], picklebrain: [], paddles: [], other: [],
              };
